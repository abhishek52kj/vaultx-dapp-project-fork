...
export const injected = new InjectedConnector({
  supportedChainIds: [1, 56, 97, 11155111, 1337, 31337, 5777],
});
...
export function getConnectorError(error) {
  ...
  if (error instanceof UnsupportedChainIdError)
    return 'Unsupported network. Please switch to Ethereum, BNB Chain, Sepolia, BSC Testnet, or a local dev chain.';
...
export function useWalletLifecycle() {
  ...
  useEffect(() => {
    let cancelled = false;
    const shouldReconnect = localStorage.getItem('connected') === 'true';

    if (shouldReconnect && window.ethereum) {
      injected
        .isAuthorized()
        .then((authorized) => {
          if (cancelled) return;
